...
void _emul_send_key(uint16_t keycode) {
    switch (keycode) {
        ...
        case AZ_EURO:
            ...
            else UNSHIFT(ALTGR(XKEY(X_E)));
            break;
// mes ajouts
        case AZ_DLRS:
            if (_emul_os_mode == EMUL_OS_OSX) OPTION(XKEY(X_RBRC))
            else UNMOD(WINALT4(0,0,3,6));
            break;
        
        case AZ_EPERL:
            if (_emul_os_mode == EMUL_OS_OSX) OPTION(XKEY(X_RBRC))
            else UNMOD(WINALT4(0,0,3,8));
            break;

        case AZ_ASTER:
            if (_emul_os_mode == EMUL_OS_OSX) OPTION(XKEY(X_RBRC))
            else UNMOD(WINALT4(0,0,4,2));
            break;

        case AZ_CROI:
            if (_emul_os_mode == EMUL_OS_OSX) OPTION(XKEY(X_RBRC))
            else UNMOD(WINALT4(0,0,3,5));
            break;

        case AZ_AROB:
            if (_emul_os_mode == EMUL_OS_OSX) OPTION(XKEY(X_RBRC))
            else UNMOD(WINALT4(0,0,6,4));
            break;

        case AZ_UNDER:
            if (_emul_os_mode == EMUL_OS_OSX) OPTION(XKEY(X_RBRC))
            else UNMOD(WINALT4(0,0,9,5));
            break;

        case AZ_PERM:
            if (_emul_os_mode == EMUL_OS_OSX) OPTION(XKEY(X_RBRC))
            else UNMOD(WINALT4(0,1,3,7));
            break;

        case AZ_PLUSMOINS:
            if (_emul_os_mode == EMUL_OS_OSX) OPTION(XKEY(X_RBRC))
            else UNMOD(WINALT4(0,1,7,7));
            break;

        case AZ_BBARE:
            if (_emul_os_mode == EMUL_OS_OSX) OPTION(XKEY(X_RBRC))
            else UNMOD(WINALT3(2,2,1));
            break;

        case AZ_SUP1:
            if (_emul_os_mode == EMUL_OS_OSX) OPTION(XKEY(X_RBRC))
            else UNMOD(WINALT4(0,1,8,5));
            break;

        case AZ_MDOT:
            if (_emul_os_mode == EMUL_OS_OSX) OPTION(XKEY(X_RBRC))
            else UNMOD(WINALT4(0,1,8,3));
            break;

        case AZ_ESZET: // lettre allemande ẞ
            if (_emul_os_mode == EMUL_OS_OSX) OPTION(XKEY(X_RBRC))
            else UNMOD(WINALT4(0,2,2,3));
            break;

        case AZ_SESZET: // lettre allemande ß
            if (_emul_os_mode == EMUL_OS_OSX) OPTION(XKEY(X_RBRC))
            else UNMOD(WINALT4(7,8,3,8));
            break;

        case AZ_MACRON:
            if (_emul_os_mode == EMUL_OS_OSX) OPTION(XKEY(X_RBRC))
            else UNMOD(WINALT4(0,1,7,5));
            break;
...
// fin mes ajouts
        ...
        case AZ_SML1:  if (_emul_os_mode == EMUL_OS_OSX)  SEND_STRING(SS_TAP(X_DOT)  SS_TAP(X_EQL) SS_TAP(X_MINS)); else SEND_STRING(SS_TAP(X_DOT)  SS_TAP(X_MINS)); break;
